...
@router.post("/createStory", dependencies=[Depends(validate_token)])
def create_new_story(story: Story):
    return create_story(story)

@router.put("/updateStory", dependencies=[Depends(validate_token)])
def update_story_information(story: Story):
    return update_story(story)

@router.delete("/deleteStory/{story_id}", dependencies=[Depends(validate_token)])
